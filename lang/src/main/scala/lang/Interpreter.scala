...
object Interpreter {
  ...
  def evalStmt1(stmt: Stmt, state: State, leaf: Boolean): Either[Set[LangException], Conf] = {
    require(state._1.nonEmpty)
    val (env, mem, nl) = state
    stmt match
      case Decl(name, value)  => (env.head.contains(name), evalExpr(value, state)) match
        case (false, Right(v)) =>
          if leaf then Right(St(env.tail, mem, nl))
          else Right(St(env.head + (name -> nl) :: env.tail, mem + (nl -> v), nl + 1))
      ...
      case Assign(to, value)  => (env.head.get(to), evalExpr(value, state)) match
        case (Some(loc), Right(v)) => mem.contains(loc) match
          case true  => 
            if leaf then Right(St(env.tail, mem, nl))
            else Right(St(env, mem.updated(loc, v), nl))
      ...
      case If(cond, body)    => evalExpr(cond, state) match
        case Left(b)  => Left(b)
        case Right(c) =>
          if c then Right(Cmd(body, (env.head :: env, mem, nl))) else Right(St(state))
      case Seq(stmt1, stmt2)  => evalStmt1(stmt1, state, false) match
        case Left(b)  => Left(b)
