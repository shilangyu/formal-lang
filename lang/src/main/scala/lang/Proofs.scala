...
object Proofs {
  ...
  /** 
   * Lemmas and proofs of closedness: 
   * A program is closed if whenever evaluating Ident〈name〉 or Assign〈name, expr〉, env(name) is defined.
   */
  object Closedness {
  ...
  /** 
   * Lemmas and proofs of closedness: 
   * A program has no redeclarations if whenever evaluating Decl〈name, expr〉, env(name) is not defined.
   */
  ...
  /** 
   * Lemmas and proofs of unique ownership: 
